...
# 1. Load series
dat <- read.csv('empirical_testing/nasdaq100/data/nasdaq100.csv')
...
write.csv(bsadf_stat, "empirical_testing/nasdaq100/results/nasdaq100_bsadf_stat.csv", row.names = FALSE)

# BSADF critical values (90, 95, 99)
bsadf_cv <- as.data.frame(cv$bsadf_cv)        # keep all significance-level columns
write.csv(bsadf_cv, "empirical_testing/nasdaq100/results/nasdaq100_bsadf_cv.csv", row.names = FALSE)